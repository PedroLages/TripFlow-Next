---
name: start-story
description: Use when beginning work on a TripFlow story, when user says "start story E##-S##", or when picking the next story from sprint-status.yaml. Creates branch, story file, suggests ATDD tests, gathers context, enters plan mode.
argument-hint: "[E##-S##]"
...
2. **Check status** in `docs/implementation-artifacts/sprint-status.yaml`. Warn if not `backlog` or `ready-for-dev`.
   - **ID normalization**: `E01-S03` → strip leading zeros → key `1-3-trip-dashboard-overview`.
...
3. **Derive branch name**: `feature/` + lowercase story ID + slugified name. Strip `& ( ) , .` and filler words (and, the, a, with, for, of). Lowercase. Hyphens between words.
   - Example: `E01-S03` "Trip Dashboard Overview" → `feature/e01-s03-trip-dashboard-overview`
...
6. **Create story file** (idempotent):
   ...
   - **File does not exist**: Create using the template at `docs/implementation-artifacts/story-template.md` if it exists, or use a basic template with frontmatter (`story_id`, `story_name`, `status: in-progress`, `started: YYYY-MM-DD`, `reviewed: false`, `review_started:`, `review_gates_passed: []`) and fill in Story, Acceptance Criteria, and Tasks from the epic.
...
8. **ATDD test suggestion** (idempotent):
   - Check if `e2e/story-{id}.spec.ts` already exists.
   ...
   - **File does not exist**: Analyze the acceptance criteria content:
     ...
     - If user accepts: Generate failing acceptance tests in `e2e/story-{id}.spec.ts` using existing Playwright fixture patterns from `e2e/fixtures/`. Tests should follow RED-GREEN-REFACTOR: write minimal failing tests that map to acceptance criteria.
...
9. **Launch 3 parallel Explore agents** via Task tool:
   ...
   - **Agent 2 — Existing code patterns**: Search affected source directories (`tripflow-next/src/app/`, `tripflow-next/src/components/`, `tripflow-next/src/lib/`, `tripflow-next/src/hooks/`) for relevant patterns, types, and utilities.
   - **Agent 3 — Test patterns + UX specs**: Read test patterns from `e2e/` and `e2e/fixtures/`, plus UX design specs from `docs/planning-artifacts/ux-design-specification.md` for relevant sections.
...
12. **Assess workflow recommendation**: Based on research from Steps 9-10, determine which shipping workflow to recommend:
    ...
    **Recommend "Review first"** (`/review-story` → `/finish-story`) when ANY of:
    ...
    - Story touches multiple areas of the codebase (database + API + UI)
...
13. **Initial commit** (idempotent):
    ...
    - **Commit does not exist**: `git add docs/implementation-artifacts/{story-key}.md docs/implementation-artifacts/sprint-status.yaml` (and `e2e/story-{id}.spec.ts` if ATDD tests were created). Commit: `chore: start story E##-S##`.
...
| `src/app/trips/[tripId]/page.tsx` | `/trips/:tripId` | Trip overview |
| `src/app/trips/[tripId]/itinerary/page.tsx` | `/trips/:tripId/itinerary` | Itinerary builder |
| `src/app/trips/[tripId]/budget/page.tsx` | `/trips/:tripId/budget` | Budget tracker |
| `src/app/trips/[tripId]/voting/page.tsx` | `/trips/:tripId/voting` | Activity voting |

## TripFlow Stack Patterns

| Story content | Pattern |
|---|---|
| UI pages/routes | Next.js App Router — `src/app/` with page.tsx |
| Data fetching | TanStack Query (React Query) + Supabase client |
| State management | React hooks + TanStack Query cache |
| UI components | shadcn/ui (Radix) — `tripflow-next/src/components/ui/` |
| Custom components | `tripflow-next/src/components/` |
...
| Story ID | Branch |
|---|---|
| E01-S03 | `feature/e01-s03-trip-dashboard-overview` |
| E02-S01 | `feature/e02-s01-itinerary-timeline-view` |
