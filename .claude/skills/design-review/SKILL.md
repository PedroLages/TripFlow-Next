...
Comprehensive design review for front-end UI changes in the TripFlow travel planning application.
...
1. **Git Analysis**: Runs `git status` and `git diff` to identify changed UI files in `tripflow-next/src/`
2. **Loads Standards**: Reads `.claude/workflows/design-review/design-principles.md`
3. **Live Browser Testing**: Navigates to `http://localhost:3100`, clicks elements, hovers, types — directly controlling the browser via Playwright MCP
...
1. **Dev server running**: Check `lsof -ti:3100` — if not running, start with `cd tripflow-next && npm run dev &`
2. **Playwright MCP available**: The `mcp__playwright__browser_navigate` tool must be accessible
3. **Git changes exist**: Run `git status` to confirm there are UI changes to review in `tripflow-next/src/`

### Providing Context

When dispatching, include relevant context in the prompt:

- Which files changed (from `git diff --name-only` filtered to `tripflow-next/src/`)
...
# Claude Code:
# 1. Checks git status for changed files in tripflow-next/src/
# 2. Verifies dev server is running on port 3100
...
- **Design standards**: `.claude/workflows/design-review/design-principles.md` — TripFlow design system and tokens
- **Agent config**: `.claude/workflows/design-review/agent-config.md` — architecture documentation
- **E2E tests**: `tripflow-next/e2e/` — automated Playwright tests for CI pipeline (separate from MCP agent)

## Notes

- The agent uses **Playwright MCP** (live browser control), not `npx playwright test` (pre-written specs)
- CI pipeline uses `tripflow-next/e2e/` tests for automated checks
- The dev server must be running at `http://localhost:3100` before the agent starts
