...
Comprehensive design review for front-end UI changes in the TripFlow AI travel planning platform.
...
3. **Live Browser Testing**: Navigates to `http://localhost:3001`, clicks elements, hovers, types — directly controlling the browser via Playwright MCP
...
1. **Dev server running**: Check `lsof -ti:3001` — if not running, start with `npm run dev &`
...
- **CI test specs**: `tripflow-next/e2e/` — automated Playwright tests for CI pipeline (separate from MCP agent)
...
- The dev server must be running at `http://localhost:3001` before the agent starts
