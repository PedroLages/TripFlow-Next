...
4. **Pre-checks** (always run — fast, validates current state):
   ...
   a. `cd tripflow-next && npm run build` — STOP on failure with build errors.
   b. `cd tripflow-next && npm run lint` — STOP on failure with lint errors (if lint script exists, otherwise skip).
   c. `cd tripflow-next && npm run test` — STOP on failure. If no unit test script or no test files, note and continue.
   d. E2E tests — run smoke specs + current story's spec on Chromium only:
      ```
      cd tripflow-next && npx playwright test e2e/smoke.spec.ts e2e/story-{id}.spec.ts --project=chromium
      ```
      If the current story has no spec file in `e2e/`, run smoke specs only. STOP on failure. Do NOT run visual regression specs here — those are separate.
...
5. **Design review** (conditional, skippable on resume):
   ...
   **No UI changes**: If `git diff --name-only main...HEAD` shows NO changes in `tripflow-next/src/app/` or `tripflow-next/src/components/`:
   ...
   **UI changes detected**: Run the design review:
   ...
   a. Check dev server: `curl -s -o /dev/null -w "%{http_code}" http://localhost:3100`.
      - Not reachable → start `cd tripflow-next && npm run dev &` in background via Bash, wait up to 30s.
...
6. **Code reviews** (parallel, skippable on resume):
   ...
   **Skip conditions** (checked independently):
   ...
   Task({
     ...
     prompt: "Review story E##-S## at docs/implementation-artifacts/{key}.md. Run git diff main...HEAD for changes in tripflow-next/. Focus on architecture, security, correctness, silent failures, and TripFlow stack patterns (Next.js App Router, Supabase, TanStack Query). Score each finding with confidence (0-100).",
     description: "Code review E##-S##"
   })

   Task({
     subagent_type: "pr-review-toolkit:pr-test-analyzer",
     prompt: "Review test coverage for story E##-S## at docs/implementation-artifacts/{key}.md. Run git diff main...HEAD for changes in tripflow-next/. Map every acceptance criterion to its tests in e2e/. Review test quality, isolation, and edge case coverage. Score each finding with confidence (0-100).",
...
7. **Merge test quality findings**:
   ...
   The test analyzer agent replaces the previous inline test quality checks. Extract its AC Coverage Table and test quality findings for the consolidated report. No additional inline checks needed — the agent handles test isolation, selector quality, fixture usage, and AC coverage.
...
|---|---|
| `src/app/page.tsx` | `/` |
| `src/app/trips/[tripId]/page.tsx` | `/trips/:tripId` |
| `src/app/trips/[tripId]/itinerary/page.tsx` | `/trips/:tripId/itinerary` |
| `src/app/trips/[tripId]/budget/page.tsx` | `/trips/:tripId/budget` |
| `src/app/trips/[tripId]/voting/page.tsx` | `/trips/:tripId/voting` |
