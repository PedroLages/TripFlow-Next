---
name: finish-story
description: Use when a TripFlow story is ready to ship. Validates, creates PR. Auto-runs reviews if /review-story was not already run. Use after implementing and optionally reviewing a story.
argument-hint: "[E##-S##]"
...
4b. **If NOT reviewed** (streamlined mode):
   ...
   - Run the full review pipeline inline — same steps as `/review-story` steps 4-8:
     a. Pre-checks: `cd tripflow-next &&` build, lint, unit tests, E2E tests (smoke specs + current story spec, Chromium only)
     b. Design review (if UI changes in `tripflow-next/src/`)
     c. Code reviews — `pr-review-toolkit:code-reviewer` and `pr-review-toolkit:pr-test-analyzer` agents in parallel
...
5. **If already reviewed** (comprehensive mode):
   ...
   - **5b. Lightweight validation**:
     a. `cd tripflow-next && npm run build` — STOP on failure.
     b. `cd tripflow-next && npm run lint` — STOP on failure (if script exists).
     c. `cd tripflow-next && npm run test` — STOP on failure (if tests exist).
     d. E2E tests — run smoke specs + current story's spec on Chromium only:
        ...
        ```
        If the current story has no spec file in `e2e/`, run smoke specs only. STOP on failure.
...
9. **Archive story spec**: If `e2e/story-*.spec.ts` exists for this story, move it to `e2e/regression/`:
   ```
   git mv e2e/story-{id}.spec.ts e2e/regression/
   git commit -m "chore: archive E##-S## spec to regression"
   ```
   If no story spec exists in `e2e/`, skip this step.
...
13. **Completion output**: Display the following summary to the user.
    ...
    **Writing guidelines** (not shown to user):
    - "What's New" bullets must describe what the user can now DO, not implementation details. Plain, non-technical language. 2-5 bullets.
      - Good: "You can now view your trip overview with all activities, budget summary, and booking status at a glance"
      - Bad: "Added TripOverview component with TanStack Query hooks"
